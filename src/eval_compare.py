...
What this does (single pass over indices):
  ...
  - NEW tables:
      * tables/summary_runtime.{csv,tex}     (GPU/driver/Torch/CUDA + time)
      * tables/trainingset_overview.{csv,tex} (dataset & training args)
...
import os, re, glob, argparse, yaml, h5py, json
import numpy as np
import pandas as pd
...
# Robust Alex loader: directory / pattern / zero-padded / non-padded
def _read_alex(alex_tpl: str, idx: int):
    import numpy as np
    if os.path.isdir(alex_tpl):
        # infer zero-padding and prefer *_rho*.npy if present
        def _infer_pad_from_dir(dir_path: str, prefix: str = "test_") -> int:
            ...
            return 3
        pad = _infer_pad_from_dir(alex_tpl, "test_")
        tmpl = os.path.join(alex_tpl, f"test_{{idx:0{pad}d}}_*rho*.npy")
        if not glob.glob(tmpl.format(idx=0)) and not glob.glob(tmpl.format(idx=1)):
            tmpl = os.path.join(alex_tpl, f"test_{{idx:0{pad}d}}_*.npy")
        alex_tpl = tmpl

    pats = []
    if "{idx" in alex_tpl:
        # 1) strict
        try: pats = sorted(glob.glob(alex_tpl.format(idx=idx)))
        except Exception: pats = []
        # 2) relax suffix
        if not pats:
            try:
                base = alex_tpl.format(idx=idx)
                pats = sorted(glob.glob(re.sub(r"\.npy$", "*.npy", base)))
            except Exception:
                pats = []
        # 3) try different padding
        if not pats:
...
# ----------------------------
# Training log parsing (meta, hardware, timing)
# ----------------------------
def _parse_args_dict_from_log_line(line: str):
    if "Args:" not in line:
        return None
    try:
        blob = line.split("Args:", 1)[1].strip()
...
def _parse_training_meta(train_log_path: str):
    if not (train_log_path and os.path.isfile(train_log_path)):
        return {}

    meta = {
        "train_files": None, "val_files": None, "test_files": None,
        "batch_size": None, "target_field": None, "input_case": None,
        "keep_two_channels": None, "sample_fraction": None, "amp": None,
        "seed": None, "exclude_list": None, "include_list": None, "yaml_path": None,
        "early_stop_epoch": None, "last_epoch_seen": None,
    }
    ...
    with open(train_log_path, "r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            ...
            # Args dict
            if "Args:" in ls:
                argsd = _parse_args_dict_from_log_line(ls)
                if isinstance(argsd, dict):
                    for k in ["batch_size","target_field","input_case","keep_two_channels",
                              "sample_fraction","amp","seed","exclude_list","include_list","yaml_path"]:
                        meta[k] = meta[k] or argsd.get(k)

            # Split lines
            m = re.search(r"Split='train'\s*\|\s*files=(\d+)\s*\|\s*batch=(\d+)\s*\|\s*target='?([a-zA-Z0-9_]+)'?", ls)
            if m:
                meta["train_files"] = int(m.group(1))
                meta["batch_size"] = meta["batch_size"] or int(m.group(2))
                meta["target_field"] = meta["target_field"] or m.group(3)
            m = re.search(r"Split='val'\s*\|\s*files=(\d+)", ls)
            if m:
                meta["val_files"] = int(m.group(1))
            m = re.search(r"Train samples \(files\):\s*(\d+)", ls)
            if m and meta["train_files"] is None:
                meta["train_files"] = int(m.group(1))
            m = re.search(r"Validation samples \(files\):\s*(\d+)", ls)
            if m and meta["val_files"] is None:
                meta["val_files"] = int(m.group(1))
            m = re.search(r"\b(\d+)\s+test files\b", ls)
            if m:
                meta["test_files"] = int(m.group(1))

            # Early stopping
            m = re.search(r"Early stopping at epoch\s+(\d+)", ls)
            if m:
                meta["early_stop_epoch"] = int(m.group(1))

            # Epoch progress lines
            m = re.search(r"Epoch\s*\[(\d+)\s*/\s*(\d+)\]", ls)
            if m:
                epoch_seen.append(int(m.group(1)))

    if epoch_seen:
        meta["last_epoch_seen"] = max(epoch_seen)
    return meta

def _safe_parse_datetime(line: str):
    # Accepts e.g. "Mon Nov  3 11:33:21 PM KST 2025"
    ls = re.sub(r"\s+", " ", line).strip()
    # try multiple patterns (locale-agnostic)
    fmts = ["%a %b %d %I:%M:%S %p %Z %Y", "%a %b %d %H:%M:%S %Z %Y"]
    for fmt in fmts:
        try:
            return datetime.strptime(ls, fmt)
        except Exception:
            continue
    return None

def _parse_hw_and_time(train_log_path: str):
    """
    Returns dict with: torch, cuda, driver, cuda_visible_devices, gpu_name, gpu_count,
    start_time, end_time, runtime_hours, epochs_trained, time_per_epoch_min
    """
    out = {
        "torch": None, "cuda": None, "driver": None, "cuda_visible_devices": None,
        "gpu_name": None, "gpu_count": None,
        "start_time": None, "end_time": None, "runtime_hours": None,
        "epochs_trained": None, "time_per_epoch_min": None,
    }
    if not (train_log_path and os.path.isfile(train_log_path)):
        return out
    ...
    with open(train_log_path, "r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            ...
            # HW basics
            m = re.search(r"^Torch:\s*([0-9.]+)", ls)
            if m: out["torch"] = m.group(1)
            m = re.search(r"^CUDA:\s*([0-9.]+)", ls)
            if m: out["cuda"] = m.group(1)
            m = re.search(r"Driver Version:\s*([0-9.]+)", ls)
            if m: out["driver"] = m.group(1)
            m = re.search(r"CUDA_VISIBLE_DEVICES:\s*(.*)$", ls)
            if m: out["cuda_visible_devices"] = m.group(1).strip()
            m = re.search(r"GPU Name\[0\]:\s*(.+)$", ls)
            if m: out["gpu_name"] = m.group(1).strip()
            m = re.search(r"GPU Count:\s*(\d+)", ls)
            if m: out["gpu_count"] = int(m.group(1))

            # Start/End stamps (take first/last)
            if ls.startswith("=== [JOB STARTED]"):
                start_line = ls.split("=== [JOB STARTED]",1)[1].strip("= -")
            if ls.startswith("=== [JOB FINISHED]"):
                end_line = ls.split("=== [JOB FINISHED]",1)[1].strip("= -")

            # Early stop / epochs
            m = re.search(r"Early stopping at epoch\s+(\d+)", ls)
            if m: early_stop_epoch = int(m.group(1))
            m = re.search(r"Epoch\s*\[(\d+)\s*/\s*(\d+)\]", ls)
            if m:
                ep = int(m.group(1))
                if last_epoch_seen is None or ep > last_epoch_seen:
                    last_epoch_seen = ep

    # parse times
    if start_line:
        # extract the date part after START marker
        # e.g., "Mon Nov  3 11:33:21 PM KST 2025 on syn08 ==="
        s = re.sub(r" on .*", "", start_line).strip("= ")
        out["start_time"] = _safe_parse_datetime(s)
    if end_line:
        e = re.sub(r"\(exit=.*\)", "", end_line)
        e = re.sub(r" on .*", "", e).strip("= ")
        out["end_time"] = _safe_parse_datetime(e)

    # compute runtime
    if out["start_time"] and out["end_time"]:
        dt = out["end_time"] - out["start_time"]
        out["runtime_hours"] = round(dt.total_seconds()/3600.0, 3)

    # epochs
    epochs_trained = early_stop_epoch or last_epoch_seen
    out["epochs_trained"] = epochs_trained

    if out["runtime_hours"] is not None and epochs_trained:
        out["time_per_epoch_min"] = round((out["runtime_hours"]*60.0)/epochs_trained, 3)
...
def main():
    ...
    ap.add_argument("--alex_tpl",      type=str, required=True,
                    help="Alex NPY template or directory. Examples: "
                         "/.../test_{idx:03d}_*rho*.npy  OR  /path/to/dir (will be normalized)")
    ...
    # NEW: training log for runtime/hardware and dataset table
    ap.add_argument("--train_log",     type=str, default=None,
                    help="Training log file to extract GPU/time/epochs and dataset meta")
    ...
    # ---- Normalize --alex_tpl when a directory is provided
    ...
    best_val_from_csv = None
    best_ep_from_csv = None
    if args.loss_csv is not None:
        if os.path.isfile(args.loss_csv):
            loss_png = os.path.join(figs_dir, "loss_curve.png")
            # also capture best-epoch / best-val for runtime table
            df = pd.read_csv(args.loss_csv)
            df.columns = [c.strip().lower() for c in df.columns]
            if all(c in df.columns for c in ("epoch","val_loss")):
                df = df.sort_values("epoch").reset_index(drop=True)
                bi = int(df["val_loss"].idxmin())
                best_ep_from_csv = int(df.loc[bi, "epoch"])
                best_val_from_csv = float(df.loc[bi, "val_loss"])
    ...
    ks_pred_samples, ks_truth_samples = [], []
    ...
    def _metrics_update(p: np.ndarray, t: np.ndarray):
        ...
        n = pv.size
        n_vox += n
        s_p  += float(pv.sum());  s_t  += float(tv.sum())
        s_p2 += float(np.dot(pv, pv)); s_t2 += float(np.dot(tv, tv))
    ...
    if count_lr > 1:
        ...
        log_std = float(np.sqrt(var_lr)); log_bias = float(mean_lr)
    elif count_lr == 1:
    ...
    if n_vox > 0:
        ...
        mae  = s_abs / n_vox
        mean_p = s_p / n_vox; mean_t = s_t / n_vox
    ...
    # -------- optional: params count from model ckpt
    params_count = None
    ...
    # -------- build/save summary tables (overall/metrics/logratio)
    ...
    # -------- NEW: runtime/hardware table from train_log (+ best val from CSV)
    rt = _parse_hw_and_time(args.train_log) if args.train_log else {}
    rt_row = {
        "Model": args.label_pred,
        "GPU Name": rt.get("gpu_name", "N/A"),
        "GPU Count": rt.get("gpu_count", "N/A"),
        "Driver": rt.get("driver", "N/A"),
        "CUDA": rt.get("cuda", "N/A"),
        "Torch": rt.get("torch", "N/A"),
        "CUDA_VISIBLE_DEVICES": rt.get("cuda_visible_devices", "N/A"),
        "Start Time": rt.get("start_time", "N/A"),
        "End Time": rt.get("end_time", "N/A"),
        "Runtime (h)": rt.get("runtime_hours", "N/A"),
        "Epochs Trained": rt.get("epochs_trained", "N/A"),
        "Time/Epoch (min)": rt.get("time_per_epoch_min", "N/A"),
        "Best Val (from CSV)": f"{best_val_from_csv:.4f}" if best_val_from_csv is not None else "N/A",
        "Best Epoch (from CSV)": best_ep_from_csv if best_ep_from_csv is not None else "N/A",
    }
    runtime_df = pd.DataFrame([rt_row])
    runtime_csv = os.path.join(tabs_dir, "summary_runtime.csv")
    runtime_df.to_csv(runtime_csv, index=False)
    if args.save_latex:
        runtime_df.to_latex(os.path.join(tabs_dir, "summary_runtime.tex"), index=False, escape=False)

    # -------- NEW: training-set overview table (from train_log)
    ts_meta = _parse_training_meta(args.train_log) if args.train_log else {}
    trainingset_row = {
        "Model": args.label_pred,
        "Target": ts_meta.get("target_field", "N/A"),
        "Input Case": ts_meta.get("input_case", "N/A"),
        "Keep Two Channels": ts_meta.get("keep_two_channels", "N/A"),
        "Batch Size": ts_meta.get("batch_size", "N/A"),
        "Sample Fraction": ts_meta.get("sample_fraction", "N/A"),
        "Train Files": ts_meta.get("train_files", "N/A"),
        "Val Files": ts_meta.get("val_files", "N/A"),
        "Test Files": ts_meta.get("test_files", "N/A"),
        "AMP": ts_meta.get("amp", "N/A"),
        "Seed": ts_meta.get("seed", "N/A"),
        "Exclude List": ts_meta.get("exclude_list", "N/A"),
        "Include List": ts_meta.get("include_list", "N/A"),
        "YAML Path": ts_meta.get("yaml_path", "N/A"),
        "Early Stop Epoch": ts_meta.get("early_stop_epoch", "N/A"),
        "Last Epoch Seen": ts_meta.get("last_epoch_seen", "N/A"),
    }
    trainingset_df = pd.DataFrame([trainingset_row])
    trainingset_csv = os.path.join(tabs_dir, "trainingset_overview.csv")
    trainingset_df.to_csv(trainingset_csv, index=False)
    if args.save_latex:
        trainingset_df.to_latex(os.path.join(tabs_dir, "trainingset_overview.tex"), index=False, escape=False)
    ...
    if args.loss_csv: print(f"[DONE] Loss curve → {os.path.join(figs_dir, 'loss_curve.png')}")
    if args.train_log: print(f"[META] train_log parsed → runtime/hw + trainingset tables")
    if 'model_class' in (model_meta or {}):
        print(f"[META] model_class={model_meta.get('model_class','N/A')}")
    if 'model_path' in (model_meta or {}):
        print(f"[META] model_path={model_meta.get('model_path','N/A')}")
